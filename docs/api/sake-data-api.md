...
HTTP API `CorsConfiguration` handles admin CORS preflight responses before requests reach the Admin Lambda. Actual admin `GET`, `POST`, and `PATCH` requests remain protected by the Cognito JWT authorizer, with the existing origin, method, and header policy unchanged. The Admin Lambda does not bypass authentication or add CORS headers for `OPTIONS` requests.
