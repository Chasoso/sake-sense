...
The dedicated CloudFormation execution role is the stack service role and remains the resource mutation authority. After the one-time bootstrap associates it with `sake-sense-ai-production`, CloudFormation continues to use that role for later stack operations. The GitHub OIDC role only orchestrates the named stack/change set, uploads artifacts, and reads the one Lambda configuration needed for post-deploy verification; it cannot mutate Lambda resources. The workflow does not bypass the gate for destructive changes.
