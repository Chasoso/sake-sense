...
The public Lambda returns explicit DTOs and removes audit/timestamp internals. The admin Lambda accepts mapped fields only and is behind the HTTP API JWT authorizer plus a server-side `admin` group check. Public routes are read-only; admin routes are authenticated and write-capable.
